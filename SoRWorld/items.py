...
# Adds agents to the item pool
ITEM_NAME_TO_ID = {}
DEFAULT_ITEM_CLASSIFICATIONS = {}

count = 1
for agent in agents:
    ITEM_NAME_TO_ID[agent] = count
    DEFAULT_ITEM_CLASSIFICATIONS[agent] = ItemClassification.progression
    count+=1
for dlc_agent in dlc_agents:
    ITEM_NAME_TO_ID[dlc_agent] = count
    DEFAULT_ITEM_CLASSIFICATIONS[dlc_agent] = ItemClassification.progression
    count+=1
for filler_item in filler:
    ITEM_NAME_TO_ID[filler_item] = count
    DEFAULT_ITEM_CLASSIFICATIONS[filler_item] = ItemClassification.filler
    count+=1
#for useful_item in dataInfo["useful_items"]["currency"]:
#    ITEM_NAME_TO_ID[useful_item] = count
#    DEFAULT_ITEM_CLASSIFICATIONS[useful_item] = ItemClassification.useful
#    count+=1
...
def get_random_filler_item_name(world: SoRWorld) -> str:
...
def create_item_with_correct_classification(world: SoRWorld, name: str) -> SoRItem:
    ...
    return SoRItem(name, classification, ITEM_NAME_TO_ID[name], world.player)


# With those two helper functions defined, let's now get to actually creating and submitting our itempool.
def create_all_items(world: SoRWorld) -> None:
    ...
    for i in range(world.options.random_characters):
        n = world.random.randint(0, len(a)-1)
        startingAgents.append(a.pop(n))
         
    for agent in agents:
        newItem = world.create_item(agent)
        if (agent in world.options.starting_characters or agent in startingAgents):
            world.push_precollected(newItem)
            continue # Prevents starting agents from being in the pool.
    ...
    if world.options.dlc_enabled:
        ...
        for dlc_agent in dlc_agents:
            itempool.append(world.create_item(dlc_agent))
